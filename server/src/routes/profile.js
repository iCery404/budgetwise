...
router.get("/", async (req, res) => {
  const [[me]] = await pool.query("SELECT id, name, email, role, created_at FROM users WHERE id = ?", [req.user.id]);
  const [pending] = await pool.query(
    "SELECT id, requested_name, requested_email, status, created_at FROM profile_requests WHERE user_id = ? AND status = 'pending' ORDER BY created_at DESC LIMIT 1",
    [req.user.id]
  );
